...
Mathcad worksheet (``# mcdx region <id>``, plus a renamed target's original
Mathcad name).
...
def test_source_id_is_always_populated_regardless_of_the_flag():
    ...
    # (flattened) area, then a trailing text and the final evaluation.
    kinds_and_ids = [(type(r).__name__, r.source_id) for r in ws.regions]
...
def test_source_comment_returns_none_without_source_id():
